...
gulp.task('sass-compile', (done) => {
	return gulp
		...
		.pipe(
			gulp.dest((file) =>
				file.base.replace('/src', '/assets').replace('/scss', '/css')
			)
...
gulp.task('js-compile', (done) => {
	return (
		gulp
			...
			.pipe(gulp.dest((file) => file.base.replace('/src', '/assets'))) // put the js files here.
			.pipe(
				gulpFn(function (file) {
...
gulp.task('start-browsersync', function (done) {
	browserSync.init({
		...
		proxy: 'http://localhost:8888/', // on the root of the project
	});
	done();
});
